...
namespace EV3RemoteControlWifi
{
    public partial class Mainform : Form
    {
        ...
        {
            // Send message "Open" to EV3.
            if (myEV3.isConnected)
            {
                myEV3.SendMessage("Open", "0");  // "0" means EV3_INBOX0
            }
        }

        private void button2_Click(object sender, EventArgs e)
        {
            // Send message "Close" to EV3.
            if (myEV3.isConnected)
            {
                myEV3.SendMessage("Close", "0");  // "0" means EV3_INBOX0
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (myEV3.isConnected)
            {
                myEV3.SendMessage("Start", "0");  // "0" means EV3_INBOX0
            }
        }
        private void receivedMessagesListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (seconden >= 59)
            {
                seconden = -1;
                minuten = minuten + 1;
                label2.Text = minuten.ToString();
            }
            if (seconden < 9)
            {
                seconden = seconden + 1;
                label5.Text = "0" + seconden.ToString();
            }
            else
            {
                seconden = seconden + 1;
                label5.Text = seconden.ToString();
            }
        }
    }
}
